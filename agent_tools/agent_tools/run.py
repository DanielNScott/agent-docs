...
from agent_tools.output import (print_resource_tree, print_annotated_tree,
                                print_call_graph, print_reverse_deps, print_def_use)
from agent_tools.output_html import generate_html, generate_annotated_html


def main():
    parser = argparse.ArgumentParser(description='Code analysis: resource tree, call graph, def-use, reverse deps')
    ...
    parser.add_argument('--no-color', action='store_true',
                       help='Disable color output for --compare')
    parser.add_argument('--callgraph', action='store_true',
                       help='Call graph only')
    parser.add_argument('--defuse', action='store_true',
                       help='Def-use analysis only')
    parser.add_argument('--revdeps', action='store_true',
                       help='Reverse dependencies only')
    parser.add_argument('--tree', action='store_true',
                       help='Resource tree only')
    parser.add_argument('--output', type=str,
                       help='Output file path for HTML modes')
    parser.add_argument('directory', nargs='?', default='.',
                       help='Directory to analyze (or first directory for --compare)')
    parser.add_argument('directory2', nargs='?',
                       help='Second directory for --compare')
    ...
    if args.compare:
        ...
        parser.error('Must specify --text, --html, --compare, or an individual analysis flag')


def _enter_project_dir(directory):
    """Validate directory, chdir into it, and discover modules.

    Returns (modules, original_dir) on success, (None, None) on failure.
    Caller is responsible for restoring original_dir when done."""
    ...
    return modules, original_dir


def run_text(args):
    """Produce all text analyses: resource tree, call graph, def-use, reverse deps."""
    modules, original_dir = _enter_project_dir(args.directory)
    if modules is None:
        return
...
def run_html(args):
    ...
    if not os.path.isabs(output_path):
        output_path = os.path.join(original_dir, output_path)

    os.chdir(target_dir)
    try:
        modules = discover_module_files()
        generate_html(modules, output_path)
...
def run_compare(args):
    ...
    os.chdir(dir1)
    modules1 = discover_module_files()
    map1 = build_definition_map(modules1)

    os.chdir(dir2)
    modules2 = discover_module_files()
