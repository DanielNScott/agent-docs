...
def generate_html(modules, output_path='API.html'):
...
def _generate_html_toc_file(file_path):
    ...
    if definitions:
        ...
        for node in definitions:
            ...
            print(f'    <li><a href="#{func_anchor}" class="func-link">{sig_html}</a></li>')
        print('  </ul>')
        ...
        for module in sorted(diff.keys()):
            ...
            for filename in sorted(files.keys()):
                ...
                for defn in file_info['definitions']:
                    ...
                    print(f'<span class="marker">{def_marker}</span>{escape_html(text)}')
                    print('</div>')
